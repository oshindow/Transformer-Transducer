...
def subsequent_mask(size, right=0, device="cpu", dtype=datatype):
    ...
    if is_torch_1_2 and dtype == torch.bool:
        ...
        return torch.tril(ret, out=ret, diagonal=right).type(dtype)
    else:
        ret = torch.ones(size, size, device=device, dtype=dtype)
        return torch.tril(ret, out=ret, diagonal=right)


def target_mask(ys_in_pad, ignore_id, right=0):
    ...
    m = subsequent_mask(ys_mask.size(-1), right, device=ys_mask.device).unsqueeze(0)
