...
class Dataset:
    def __init__(self, config, type):
        ...
        self.arkscp = os.path.join(config.__getattr__(type), 'cmvn-feats.scp')
    ...
    def pad(self, inputs, max_length=None):
        ...
        if dim == 1:
            ...
            pad_zeros_mat = - np.ones([1, max_length - inputs.shape[0]], dtype=np.int32)
    ...
    def concat_frame(self, features):
        ...
        concated_features = np.delete(concated_features, range(self.left_context_width), axis=0)
        concated_features = np.delete(concated_features, [(x + concated_features.shape[0] - self.right_context_width) for x in range(self.right_context_width)], axis=0)
...
class AudioDataset(Dataset):
    def __init__(self, config, type):
        ...
        self.config = config
        # self.text = os.path.join(config.__getattr__(type), 'char.txt')
        # if timit
        ...
        # self.targets_dict = self.get_targets_text()

        # sort uttid by sequence length
    ...
    def get_vocab_map(self):
        ...
        with codecs.open(self.vocab, 'r', encoding='utf-8') as fid:
            for line in fid:
                parts = line.strip().split()
                #unit2idx["".join(parts)] = idx
                unit2idx[parts[0]] = idx
                idx = idx + 1
        return unit2idx

    def get_targets_text(self):
        targets_str = {}
        with codecs.open(self.text, 'r', encoding='utf-8') as fid:
            for line in fid:
                ...
                if len(contents) < 0 or len(contents) > self.max_target_length:
                    continue
                targets_str[utt_id] = contents
        return targets_str

    # transform a sym seq to idx seq
