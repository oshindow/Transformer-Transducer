...
from tt.common import get_vgg2l_odim
from tt.utils.net_utils import make_pad_mask
...
class RNNEncoder(torch.nn.Module):
    ...
    def __init__(
        self, etype, elayers, eunits, eprojs, idim, subsample, dropout_rate, in_channel=1):
        ...
        if etype.startswith("vgg"):
            ...
            if etype[-1] == "p":
                self.enc = torch.nn.ModuleList(
                    [RNNP(idim, elayers, eunits, eprojs, subsample, dropout_rate, typ=typ)]
                )
                logging.info(typ.upper() + " with every-layer projection for encoder")
            else:
                self.enc = torch.nn.ModuleList(
                    [RNN(idim, elayers, eunits, eprojs, dropout_rate, typ=typ)]
                )
